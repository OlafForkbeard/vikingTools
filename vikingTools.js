...
vt.version = "v1.000";
//Debug Modes
//When True: Reports everything to the console in addition to the normal log.
vt.logConsoleDebugMode = false;
//When True: Reports everything to the alert() system.
vt.logAlertDebugMode = false;
//Declare the logfile, and set node 0 to the starting log data.
vt.logfile = ["vikingToolbox.js v" + vt.version + " was initiated: " + Date.now()];
vt.log = function(logDataInput, consoleDebugMode, alertDebugMode) {
  ...
  //debug every time.
  if (vt.logConsoleDebugMode === true) {
    consoleDebugMode = true;
  }
  //If any debug mode is on, ignore sent parameter for it, and just 
  //debug every time.
  if (vt.logAlertDebugMode === true) {
    alertDebugMode = true;
  }
  //Catches for undefined and false sents. Defaults to debug off.
  if (consoleDebugMode === undefined || consoleDebugMode === false || logDebugInput === 0) {
    alert("log() consoleDebugMode === undefined || consoleDebugMode === false || logDebugInput === 0");
    //Take no action
  } else if (consoleDebugMode === true || consoleDebugMode === 1) {
  ...
  //Catches for undefined and false sents. Defaults to debug off.
  if (logAlertMode !== undefined || logAlertMode !== false || logAlertMode !== 0) {
    alert(logDataOutput);
  } else {
    //Logdump, unknown error
...
//      location regardless of code execution then 
//      fires a function. Alternatively it can halts 
//      all code execution for up to 9999 ms.
